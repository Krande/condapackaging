...
def validate_version(s):
    """Ensure that s contains an X.Y.Z or X.Y.Z.z format version string, or raise ValueError."""
    # Adjusted regex to optionally match a fourth number
    m = re.match(r'(\d+)\.(\d+)\.(\d+)(?:\.(\d+))?$', s)
    if m:
        return tuple(int(x) for x in m.groups() if x is not None)
    raise ValueError(f"HDF5 version string {s!r} not in X.Y.Z or X.Y.Z.z format")
