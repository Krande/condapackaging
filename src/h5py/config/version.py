...
def format_version(version_tuple_):
    """Format the version tuple into a version string."""
    if len(version_tuple_) == 3:
        return "%d.%d.%d" % version_tuple_
    elif len(version_tuple_) == 4:
        return "%d.%d.%d.%d" % version_tuple_
    else:
        raise ValueError(f"Version tuple {version_tuple_} is not in a valid format")
...
version_tuple = _H5PY_VERSION_CLS(3, 11, 1, None, None, None)
...
hdf5_version = format_version(hdf5_version_tuple)
...
""" % {
    ...
    'hdf5_build_version': format_version(hdf5_built_version_tuple),
}
